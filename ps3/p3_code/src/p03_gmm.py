...
def main(is_semi_supervised, trial_num):
    ...
    mu, sigma = [],[]
    gaussians = np.array_split(x, K)
    for i in range(K):
        mu_i = np.mean(gaussians[i], axis=0)
        sigma_i = np.cov(gaussians[i], rowvar=False)
        mu.append(mu_i)
...
def normal(x, mu, sigma):
    m = x.shape[0]
    return  np.exp(-1/2 * (x - mu).T.dot(np.linalg.inv(sigma)).dot(x - mu) / np.sqrt(np.linalg.det(sigma)))
